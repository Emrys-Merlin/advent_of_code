...
def mark_axis_parallels(start_points: np.ndarray, end_points: np.ndarray) -> np.ndarray:
...
def find_dangerous_spots(start_points: np.ndarray, end_points: np.ndarray) -> int:
...
def main(path: Union[str, Path]):
    ...
    with open(path, "r") as f:
        for i, line in tqdm(enumerate(f.readlines())):
            ...
            start_points.append([int(char) for char in start.split(",")])
            end_points.append([int(char) for char in end.split(",")])
